...
| Dispatcher Lambda | `heediq-transcription-dispatcher` (Node 22) — sole consumer of `heediq-transcription` (SQS event source, batchSize=1, reportBatchItemFailures); reads `tier` from the message **body** and calls `ecs:RunTask` on the matching task-def **family**, passing the raw body as the `SQS_MESSAGE_BODY` container override on the EC2 capacity provider (D-157, replaces the two EventBridge Pipes). Code: `lib/transcription/dispatcher/` (`index.mjs` handler + `routing.mjs` pure logic) |
...
| IAM dispatcher role | `heediq-transcription-dispatcher`'s Lambda role — SQS consume (via event source) + `ecs:RunTask` scoped to `heediq-transcription-{free,paid}:*` + `iam:PassRole` on the execution + task roles (D-157) |
| ECR images | `313828097088.dkr.ecr.eu-west-1.amazonaws.com/heediq-worker-transcription:{free\|paid}-sha-<7chars>` — two per-tier images, sha-tagged (D-047, D-062); image tag promoted per-environment by CI via `aws ssm put-parameter` + `ecs register-task-definition` (no pipe/target update step — the dispatcher runs tasks by **family**, so a newly-registered revision is picked up automatically, D-157); lifecycle rule keeps only the **last 3 images per tier** plus untagged layers ≤1 day (D-108, supersedes D-101's keep-count) — the `tagPrefixList` must match the actual `free-sha-`/`paid-sha-` prefixes CI pushes, not a bare `sha-` prefix, or nothing expires. CI builds push with `provenance: false`/`sbom: false` (D-108) — buildx's default attestation manifests were leaking untagged images the lifecycle rule couldn't reach (they stay index-referenced by the tag) |

**Message routing (D-059, D-060, D-062, D-157):** The API enqueues a `TranscriptionJobMessage` on the single `heediq-transcription` queue with `tier` in the message **body** (no `tier` SQS message attribute — routing is content-based in the dispatcher, not filter-based). The dispatcher Lambda reads `tier` and launches the matching task-def family via `RunTask`. Access to `paid` (large-v3) is enforced at the API enqueue endpoint — free users are rejected if they request it. No idle containers — tasks launch on demand, EC2 instance terminates after job completes. No `TIER` env var in task definitions — tier is per-image, not env-based (D-062).

**Spot interruption (D-066):** The dispatcher deletes the SQS message the moment it hands the job to `RunTask` — before the worker process starts. There is no visibility-timeout left to expire by the time a Spot SIGTERM arrives. Worker catches SIGTERM → writes `status=retrying` → **explicitly re-enqueues** the `TranscriptionJobMessage` to `heediq-transcription` (tier travels in the body, so the re-enqueued job re-routes correctly with no message attribute needed).
...
- **OIDC trust policy `sub` must be a wildcard, and must trust BOTH subject formats** — the deploy/ECR role trusts (`scripts/setup.sh`) list `repo:heediq/*:*` **and** `repo:heediq@294404675/*:*` with `StringLike`. GitHub now defaults brand-new repos to the *immutable* OIDC subject format `repo:heediq@<orgId>/<repo>@<repoId>:...` (older repos still emit the legacy `repo:heediq/<repo>:...`), so a single-pattern trust silently blocks every new repo's first deploy with `Not authorized to perform sts:AssumeRoleWithWebIdentity` even though test/build pass — this bit `heediq-chat` on 2026-07-22. Check a repo's format with `gh api repos/heediq/<repo>/actions/oidc/customization/sub` (the `@<id>` in `sub_claim_prefix`). Locking to a branch (`ref:refs/heads/develop`) also blocks PRs and feature-branch synths. Re-run `scripts/setup.sh` (all four accounts) if the trust policy drifts (idempotent); a new immutable-format repo also needs its own numeric ids covered by the org-level `repo:heediq@<orgId>/*:*` pattern, which already matches any repo under the org.
